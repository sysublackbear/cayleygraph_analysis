...
type Namer interface {
	// ValueOf Given a node ID, return the opaque token used by the QuadStore
	// to represent that id. Returns nil, nil on not found.
	// 给定一个id，返回在图中存储的token映射(quad.Value -> refs.Ref)
	ValueOf(quad.Value) (Ref, error)
	// NameOf Given an opaque token, return the node that it represents.
	// Returns nil, nil on not found. (refs.Ref -> quad.Value)
	// 给定一个图中的token，返回特定的id
...
func ValuesOf(ctx context.Context, qs Namer, vals []Ref) ([]quad.Value, error) {
	if bq, ok := qs.(BatchNamer); ok {
		return bq.ValuesOf(ctx, vals)  // 看能不能转成BatchNamer,如果可以，直接转换
	}
	// 否则，for循环转换
