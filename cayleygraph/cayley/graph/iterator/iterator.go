...
type Base interface {
	...
	// TagResults Fills a tag-to-result-value map.
...
type Shape interface {
	...
	// Stats These methods relate to choosing the right iterator, or optimizing an
	// iterator tree
	//
	...
	// thing, and not exact, but a useful d.
	// Stats 返回调用该遍历方法所需要的开销
	Stats(ctx context.Context) (Costs, error)

	// Optimize Optimizes an iterator. Can replace the iterator, or merely move things
	...
	// 对迭代器进行表达优化，第二个参数代表经过优化后，是否替代了原来的迭代器
