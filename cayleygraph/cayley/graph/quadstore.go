...
type QuadIndexer interface {
	// Quad : Given an opaque token, returns the quad for that token from the store.
	// 通过一个不透明的token,从图存储中找出对应的值
	Quad(Ref) (quad.Quad, error)

	// QuadIterator Given a direction and a token, creates an iterator of links which have
	// that node token in that directional field.
	// 根据方向和对应的token值，返回一个迭代器
	...
	// QuadDirection Convenience function for speed. Given a quad token and a direction
...
type QuadStore interface {
	...
	// ApplyDeltas The only way in is through building a transaction, which
	...
	// NodesAllIterator Returns an iterator enumerating all nodes in the graph.
	// 返回图中所有节点的列表的迭代器
	NodesAllIterator() iterator.Shape

	// QuadsAllIterator Returns an iterator enumerating all links in the graph.
	// 返回图中所有的边的列表的迭代器
